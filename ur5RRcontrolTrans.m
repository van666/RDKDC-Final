function [finalerr] =ur5RRcontrolTrans(gdesired, K, ur5)
...
%     q = [1.1310; -0.6283; 1.0053; 0.5027; -0.1885; 0.4398];
    ...
    while 1
        ...
        p1=norm(g_recent(1:3,4)-gdesired(1:3,4));
        if p1 <= 0.22&& p1>0.065
            Tstep=p1/3;
            t=1.5;
        elseif p1<=0.065 && p1>=0.045
            Tstep=p1/15;
            t=1;
        elseif p1<0.045
            Tstep=p1/5;
        ...
        dq = K*Tstep*transpose(ur5BodyJacobian(q))*twist;
        

        if max(dq)/t>0.5*pi*ur5.speed_limit
            disp("````````reduce speed````````")
            dq=0.5*dq/(max(dq)/(t*pi*ur5.speed_limit));
